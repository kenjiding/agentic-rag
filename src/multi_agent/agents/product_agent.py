...
def clean_messages_for_llm(messages: List[BaseMessage], keep_recent_n: int = 10) -> List[BaseMessage]:
    ...
    for msg in recent_messages:
        if isinstance(msg, ToolMessage):
            ...
            if hasattr(msg, "tool_call_id") and msg.tool_call_id in valid_tool_call_ids_from_ai:
                ...
                logger.debug(f"[PRODUCT_AGENT] 移除孤立的 ToolMessage: tool_call_id={getattr(msg, 'tool_call_id', 'N/A')}")
    ...
    if orphaned_tool_calls_count > 0:
        logger.warning(f"[PRODUCT_AGENT] 移除了 {orphaned_tool_calls_count} 个孤立的 tool_calls")
    if orphaned_tool_messages_count > 0:
        logger.warning(f"[PRODUCT_AGENT] 移除了 {orphaned_tool_messages_count} 个孤立的 ToolMessage")
