...
def benchmark_queries(rag: AgenticRAG, queries: list, iterations: int = 3):
    """
    基准测试
    ...
    for query in queries:
        ...
        for i in range(iterations):
            ...
            results["total_time"].append(duration)
            results["iteration_counts"].append(result["iteration_count"])
            results["quality_scores"].append(result.get("answer_quality", 0.0))
            results["retrieval_qualities"].append(result.get("retrieval_quality", 0.0))
            
            print(f"  迭代 {i+1}: {duration:.2f}s, "
                  f"迭代次数={result['iteration_count']}, "
                  f"质量={result.get('answer_quality', 0.0):.2f}")
    
    print("\n" + "=" * 60)
    print("基准测试结果")
    print("=" * 60)
    print(f"平均响应时间: {mean(results['total_time']):.2f}s")
    print(f"平均迭代次数: {mean(results['iteration_counts']):.2f}")
    print(f"平均答案质量: {mean(results['quality_scores']):.2f}")
    print(f"平均检索质量: {mean(results['retrieval_qualities']):.2f}")
    print("=" * 60)
...
def main():
    ...
    results = benchmark_queries(rag, test_queries, iterations=2)
